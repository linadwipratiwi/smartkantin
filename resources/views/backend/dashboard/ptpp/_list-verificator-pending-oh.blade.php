        ...
        <div class="panel panel-default card-view">
            ...
            <div class="panel-wrapper collapse in">
                <div class="panel-body">
                    @if($list_ptpp_verificator_oh->count())
                    <div class="table-wrap">
                        <div class="table-responsive">
                            <table id="datatable" class="table table-hover display  pb-30" >
                                ...
                                <tbody>
                                    @foreach($list_ptpp_verificator_oh as $row => $verification)
                                    <tr id="tr-{{$verification->ptpp->id}}">
                                        ...
                                        <td>{!! $verification->ptpp->statusFollowUp('verificator') !!}</td>
                                        <td>
                                            {{-- Approval to OH --}}
                                            ...
                                            <a onclick="reject('{{url('reject')}}', 'PTPPVerificator', {{$verification->id}}, {{$verification->approval_to_oh}}, 'status_approval_to_oh', 'notes_approval_to_oh', 'approval_at_oh', '{{url('/')}}')" data-toggle="modal" data-target=".detail-modal" data-toggle="tooltip" data-original-title="Close">
                    ...
                    @else TIDAK ADA FORM VERIVICATOR PTPP YANG BUTUH PERSETUJUAN ANDA @endif
                </div>
            </div>
        </div>	
    </div>
