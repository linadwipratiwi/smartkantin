...
<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title" >{{$ptpp_file->ptpp->name}}</h4>
        </div>
        <form id="form-file" enctype="multipart/form-data">
            <input name="_method" type="hidden" value="PUT">
            <div class="col-lg-12" id="form-item">
                {!! csrf_field() !!}
                <input type="hidden" id="ptpp_id" name="ptpp_id" value="{{$ptpp_file->ptpp->id}}">
                <div class="form-group mt-20 ">
                    <label class="control-label mb-10">{!! label('nama file pendukung', 'name') !!}</label>
                    <input type='text' name="file_name" id="name" required value="{{$ptpp_file->name}}" class="form-control" />
                </div>
                <div class="form-group mt-20 ">
...
<script>
    ...
    $("#form-file").on('submit', (function(ev) {
        ...
        $.ajax({
            ...
            url: '{{url("ptpp/file/".$ptpp_file->id)}}',
            method: 'POST',
