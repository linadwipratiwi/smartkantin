...
<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title" >{{$submission_file->submission->name}}</h4>
        </div>
        <form id="form-file" enctype="multipart/form-data">
            <input name="_method" type="hidden" value="PUT">
            <div class="col-lg-12" id="form-item">
                {!! csrf_field() !!}
                <input type="hidden" id="submission_id" name="submission_id" value="{{$submission_file->submission->id}}">
                <div class="form-group mt-20 ">
                    <label class="control-label mb-10">{!! label('nama file pendukung', 'name') !!}</label>
                    <input type='text' name="file_name" id="name" required value="{{$submission_file->name}}" class="form-control" />
                </div>
                <div class="form-group mt-20 ">
                    <label class="control-label mb-10">File</label>
                    <input type='file' name="file" id="file" class="form-control" />
                    {!! $submission_file->link() !!}
                </div>
            </div>
...
<script>
    ...
    $("#form-file").on('submit', (function(ev) {
        ...
        $.ajax({
            ...
            url: '{{url("submission/file/".$submission_file->id)}}',
            method: 'POST',
