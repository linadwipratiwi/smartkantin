    ...
    <div class="modal-content">
        <div class="modal-header">
            ...
            <div class="pull-right">
                <button class="btn btn-info btn-lable-wrap left-label" onclick="sendEmail()">
                    <span class="btn-label btn-label-checklist"><i class="fa fa-external-link"></i> </span>
                    <span class="btn-text btn-text-checklist">Kirim Request Approval</span>
                </button>
            </div>
        </div>
        <div class="col-lg-12">
                    ...
                    <div class="col-sm-6">
                        <div class="panel panel-default card-view">
                            ...
                            <div class="panel-wrapper collapse in">
                                <div class="panel-body panel-body-checklist">
                                    @if($maintenance_activity->status_approval == 0)
                                        <button class="btn btn-info btn-lable-wrap left-label" onclick="sendRequest('{{url("checklist/".$maintenance_activity->id."/send-request-checklist")}}', '{{url("checklist/".$maintenance_activity->id."/cancel-request-checklist")}}', 'checklist')">
                                            <span class="btn-label btn-label-checklist"><i class="fa fa-external-link"></i> </span>
                                            <span class="btn-text btn-text-checklist">Kirim Request Approval</span>
                                        </button>
                                    @elseif($maintenance_activity->status_approval == 1)
                                        <button class="btn btn-warning btn-lable-wrap left-label" onclick="cancelRequest('{{url("checklist/".$maintenance_activity->id."/cancel-request-checklist")}}', '{{url("checklist/".$maintenance_activity->id."/send-request-checklist")}}', 'checklist')">
                                            <span class="btn-label btn-label-checklist"><i class="fa fa-close"></i> </span>
                                            <span class="btn-text btn-text-checklist">Batalkan Request Approval</span>
                                        </button>
                                    @else
                                    <div class="text-left">
...
function sendEmail() {
    location.href='{{url("checklist/".$maintenance_activity->id."/send-email")}}';
}
</script>
